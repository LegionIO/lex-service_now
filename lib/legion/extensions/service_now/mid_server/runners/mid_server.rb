        ...
        module Runners
          module MidServer
            ...
            def list_mid_servers(sysparm_limit: 100, sysparm_offset: 0,
              ...
              resp = connection(**).get('/api/now/table/ecc_agent', params)
              { mid_servers: resp.body['result'] }
            end

            def get_mid_server(sys_id:, **)
              resp = connection(**).get("/api/now/table/ecc_agent/#{sys_id}")
            ...
            def update_mid_server(sys_id:, status: nil, validated: nil, **)
              ...
              resp = connection(**).patch("/api/now/table/ecc_agent/#{sys_id}", body)
              { mid_server: resp.body['result'] }
            end
            ...
            def list_mid_server_capabilities(mid_server_sys_id:, **)
              ...
              resp = connection(**).get('/api/now/table/ecc_agent_capability_m2m', params)
