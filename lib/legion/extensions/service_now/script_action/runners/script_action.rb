        ...
        module Runners
          module ScriptAction
            ...
            def list_script_actions(sysparm_limit: 100, sysparm_offset: 0,
              ...
              resp = get('/api/now/table/sysevent_script_action', params, **)
              { script_actions: resp.body['result'] }
            end

            def get_script_action(sys_id:, **)
              resp = get("/api/now/table/sysevent_script_action/#{sys_id}", {}, **)
            ...
            def create_script_action(name:, event_name:, script:, active: true, **)
              ...
              resp = post('/api/now/table/sysevent_script_action', body, **)
              { script_action: resp.body['result'] }
            end
            ...
            def update_script_action(sys_id:, script: nil, active: nil, **)
              ...
              resp = patch("/api/now/table/sysevent_script_action/#{sys_id}", body, **)
              { script_action: resp.body['result'] }
            end

            def delete_script_action(sys_id:, **)
              resp = delete("/api/now/table/sysevent_script_action/#{sys_id}", **)
