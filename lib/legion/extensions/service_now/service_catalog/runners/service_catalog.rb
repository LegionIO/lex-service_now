        ...
        module Runners
          module ServiceCatalog
            ...
            def list_catalogs(sysparm_limit: 100, sysparm_offset: 0, **)
              ...
              resp = get('/api/sn_sc/servicecatalog/catalogs', params, **)
              { catalogs: resp.body['result'] }
            end

            def get_catalog(sys_id:, **)
              resp = get("/api/sn_sc/servicecatalog/catalogs/#{sys_id}", {}, **)
              { catalog: resp.body['result'] }
            end

            def get_category(sys_id:, **)
              resp = get("/api/sn_sc/servicecatalog/categories/#{sys_id}", {}, **)
            ...
            def list_items(sysparm_limit: 100, sysparm_offset: 0, sysparm_query: nil,
              ...
              resp = get('/api/sn_sc/servicecatalog/items', params, **)
              { items: resp.body['result'] }
            end

            def get_item(sys_id:, **)
              resp = get("/api/sn_sc/servicecatalog/items/#{sys_id}", {}, **)
              { item: resp.body['result'] }
            end

            def get_item_variables(sys_id:, **)
              resp = get("/api/sn_sc/servicecatalog/items/#{sys_id}/variables", {}, **)
            ...
            def order_now(sys_id:, quantity: 1, variables: nil, **)
              ...
              resp = post("/api/sn_sc/servicecatalog/items/#{sys_id}/order_now", body, **)
            ...
            def add_to_cart(sys_id:, quantity: 1, variables: nil, **)
              body = { quantity: quantity }
              body[:variables] = variables if variables
              resp = post("/api/sn_sc/servicecatalog/items/#{sys_id}/add_to_cart", body, **)
              { cart: resp.body['result'] }
            end

            def get_cart(**)
              resp = get('/api/sn_sc/servicecatalog/cart', {}, **)
              { cart: resp.body['result'] }
            end

            def checkout_cart(**)
              resp = post('/api/sn_sc/servicecatalog/cart/checkout', {}, **)
              { order: resp.body['result'] }
            end

            def delete_cart(cart_id:, **)
              resp = delete("/api/sn_sc/servicecatalog/cart/#{cart_id}", **)
