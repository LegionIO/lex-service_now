        ...
        module Runners
          module SystemProperty
            ...
            def list_properties(sysparm_limit: 100, sysparm_offset: 0,
              ...
              resp = connection(**).get('/api/now/table/sys_properties', params)
              { properties: resp.body['result'] }
            end

            def get_property(sys_id:, **)
              resp = connection(**).get("/api/now/table/sys_properties/#{sys_id}")
            ...
            def create_property(name:, value:, description: nil, type: 'string', **)
              ...
              resp = connection(**).post('/api/now/table/sys_properties', body)
              { property: resp.body['result'] }
            end

            def update_property(sys_id:, value:, **)
              resp = connection(**).patch("/api/now/table/sys_properties/#{sys_id}", { value: value })
              { property: resp.body['result'] }
            end

            def delete_property(sys_id:, **)
              resp = connection(**).delete("/api/now/table/sys_properties/#{sys_id}")
