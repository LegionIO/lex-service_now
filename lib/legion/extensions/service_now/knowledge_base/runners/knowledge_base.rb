        ...
        module Runners
          module KnowledgeBase
            ...
            def list_knowledge_bases(sysparm_limit: 100, sysparm_offset: 0,
              ...
              resp = connection(**).get('/api/now/table/kb_knowledge_base', params)
              { knowledge_bases: resp.body['result'] }
            end

            def get_knowledge_base(sys_id:, **)
              resp = connection(**).get("/api/now/table/kb_knowledge_base/#{sys_id}")
            ...
            def create_knowledge_base(title:, description: nil, owner: nil,
              ...
              resp = connection(**).post('/api/now/table/kb_knowledge_base', body)
              { knowledge_base: resp.body['result'] }
            end
            ...
            def update_knowledge_base(sys_id:, title: nil, description: nil,
              ...
              resp = connection(**).patch("/api/now/table/kb_knowledge_base/#{sys_id}", body)
              { knowledge_base: resp.body['result'] }
            end

            def delete_knowledge_base(sys_id:, **)
              resp = connection(**).delete("/api/now/table/kb_knowledge_base/#{sys_id}")
            ...
            def list_kb_categories(knowledge_base_sys_id:, sysparm_limit: 100, **)
              ...
              resp = connection(**).get('/api/now/table/kb_category', params)
